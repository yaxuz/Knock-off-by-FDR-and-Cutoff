...
Generate_statistics_res_all_complications <- function(outcome,nfolds=10,train_date_end="2017-03-01",column_sums=c(5,10,15,20),fdr_col=c(0,1,0.12,0.15,0.2),offset=1,parallelcores ="max",data_type="Rdata",
                                        data_path="/Volumes/UCHealthSOR/Data/Data_20210113/Bleeding_2022_0104/",
                                        fileName ="complications",save_res=TRUE,save_dir="/Users/yaxuzhuang/OneDrive - The University of Colorado Denver/Katie Project/2022_1011/All_comp/res/")
{
  ...
  AMA_path <- substr(data_path,1,nchar(data_path)-38)
  ...
  AMA <- read.sas7bdat(file = "/Volumes/UCHealthSOR/Data/AMA/cptcodes2020.sas7bdat")
  #dim(AMA)
  #colnames(AMA)
  ...
  for (i in 1:length(column_sums)) {
    ...
    #nfolds <- 10
    set.seed(1234)
    W = stat.glmnet_coefdiff(l, X_k, y=training_set[,outcome], nfolds=nfolds, family="binomial",cores = numCores)
    #hist(W,breaks = 100)
    #hist(W)
    Sys.sleep(0.1)
    # update progress bar
    ...
    for (j in 1:length(fdr_col)) {
      ...
      train2 <- training_set[c(outcome, names(m))]
      train3 <- train2[,-1]
      glm1 <- glm(train2[,outcome] ~ ., data = train3, family="binomial")
      ...
      cmFull.1 <- as.data.frame(cbind(training_set[,outcome], fit1))
      colnames(cmFull.1) <- c(outcome, paste("Pred",outcome, sep=""))
      
      ### Training set fit
      
      roc_lasso1.1 <- roc(cmFull.1[,outcome], cmFull.1[,paste("Pred",outcome, sep="")])
      #roc_lasso1.1
      youd_lasso1.1 <- coords(roc_lasso1.1, x = "best", best.method = "youden", ret = c("threshold", "specificity", "sensitivity","accuracy", "npv", "ppv", "fn", "fp","precision","recall"))
      youd_lasso1.1$F1_stat <- 2*youd_lasso1.1$precision*youd_lasso1.1$recall/(youd_lasso1.1$precision+youd_lasso1.1$recall)
      ...
      cmFull.2 <- as.data.frame(cbind(testing_set[,outcome], fit2))
      colnames(cmFull.2) <- c(outcome, paste("Pred",outcome, sep=""))
      
      ### test set fit
      roc_lasso1.2 <- roc(cmFull.2[,outcome], cmFull.2[,paste("Pred",outcome, sep="")])
      #roc_lasso1.2
      youd_lasso1.2 <- coords(roc_lasso1.2, x = as.numeric(youd_lasso1.1$threshold), ret = c("threshold", "specificity", "sensitivity","accuracy", "npv", "ppv", "fn", "fp","precision","recall"))
      youd_lasso1.2$F1_stat <- 2*youd_lasso1.2$precision*youd_lasso1.2$recall/(youd_lasso1.2$precision+youd_lasso1.2$recall)
      ...
      #pr.curve(scores.class0 = cmFull.1[,paste("Pred",outcome, sep="")][cmFull.1[,outcome] == 1], scores.class1 = cmFull.1[,paste("Pred",outcome, sep="")][cmFull.1[,outcome] == 0], curve = T)$auc.integral
      
      comb_res_train <- round(cbind(col_cutoff,ncol(l),false_discovery_rate,(length(selected)-1),youd_lasso1.1,as.numeric(auc(roc_lasso1.1)),
                                    pr.curve(scores.class0 = cmFull.1[,paste("Pred",outcome, sep="")][cmFull.1[,outcome] == 1], scores.class1 = cmFull.1[,paste("Pred",outcome, sep="")][cmFull.1[,outcome] == 0], curve = T)$auc.integral),3)
      comb_res_train$training_or_testing <- "training"
      colnames(comb_res_train) <- c("cutoff: # of patients > "," # of candidate parameters","fdr","# of predictors selected","threshold", 
                          "specificity", "sensitivity","accuracy", "npv", "ppv", "fn", "fp","precision","recall","F1","AUC","AUC-PR","training_or_testing")
      comb_res_test <- round(cbind(col_cutoff,ncol(l),false_discovery_rate,(length(selected)-1),youd_lasso1.2,as.numeric(auc(roc_lasso1.2)),
                                   pr.curve(scores.class0 = cmFull.2[,paste("Pred",outcome, sep="")][cmFull.2[,outcome] == 1], scores.class1 = cmFull.2[,paste("Pred",outcome, sep="")][cmFull.2[,outcome] == 0], curve = T)$auc.integral),3)
      comb_res_test$training_or_testing <- "testing"
      colnames(comb_res_test) <- c("cutoff: # of patients > "," # of candidate parameters","fdr","# of predictors selected","threshold", 
                          "specificity", "sensitivity","accuracy", "npv", "ppv", "fn", "fp","precision","recall","F1","AUC","AUC-PR","training_or_testing")
  ...
  if (save_res==TRUE){
    if (is.na(save_dir)) {
      warning("When save_res is true, you need a file directory to save results, file directory will be created automaticlly.")}
    ...
    write.csv(store_saved_res_coef_statistics$result,file = paste(save_dir,outcome,Sys.Date(),".csv",sep = "_"))
  }
